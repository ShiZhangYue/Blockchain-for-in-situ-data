...
for i in range(len(layer)):
    for j in range(len(layer[i])):
        temp=bytes(layer[i][j], 'utf-8')
        layer_encrypted[i][j] = cipher.encrypt(temp)
...
blockchain = [block.create_genesis_block(data=layer_encrypted[0])]
previous_block = blockchain[0]
Hash_ori.append(blockchain[0].hash)

# after the genesis block
num_of_blocks_to_add = len(layer_encrypted)-1

# Add blocks to the blockchain
for i in range(0, num_of_blocks_to_add):
      block_to_add = block.next_block(previous_block,layer_encrypted[i+1])
...
for i in range(len(blockchain)):
    temp=[]
    for j in range(len(blockchain[i].data)):
        decrypted_message = decrypt.decrypt(blockchain[i].data[j])
        temp.append(decrypted_message)
    layer_decrypted.append(temp)
end_time=datetime.datetime.now()       
print("\n running time is ")
print(end_time-start_time)
